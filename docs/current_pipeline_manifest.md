...
- `sugar_bridge_stock.csv` 中 17 个 route-gap 衍生条目全部为 `tier_3_connectivity_only`、`keep_virtual_bridge`，没有自动提升到 strict/trusted。
- 17 个条目中 16 个为乙酰化糖片段，1 个为游离/中性糖片段；当前均缺少商业或文献证据。
...
1. 将 donor surrogate 从 `placeholder_unmapped` 重写为 atom-mapped 模板，并在 map-retention、结构身份和测试分子上通过审计后再考虑启用。
2. 对 17 个 sugar bridge 条目进行外部文献/商业证据检索；只有证据充分者才可进入 trusted/strict 候选。
3. 运行 PubChem 验证 panel 的 8/9 号虚拟桥接实验，评估 sugar bridge 对 naringin、neohesperidin、narirutin、rutin、quercitrin 的泛化。
4. 对 USPTO 保护糖 artifact 建立惩罚、归一化或过滤策略，避免保护态糖片段在报告中被误解为 donor 证据。
5. 将芳香黄酮苷叶子保持为人工审查目标，而非虚拟库存。
...
2. **17 个 sugar bridge 条目已完成保守分级，但仍缺真实证据。**
   当前全部是 `tier_3_connectivity_only` 和 `keep_virtual_bridge`。后续需要外部商业/文献证据，不能仅凭 bridge-closed 结果升级。
...
4. **真实 glycosyl donor 层仍未通过验证。**
   ...
   ```
   donor surrogate / trichloroacetimidate / bromide / thioglycoside 已有禁用占位配置，但全部未映射、map retention=0，不能进入 MCTS 结论。
...
10. **下一步真正的科学问题是“糖 donor 验证”，不是继续扩 virtual stock。**
   virtual bridge 已经证明瓶颈在糖层。再盲目加 stock 会让结果变松；现在要把糖 bridge 逐步升级成可解释的 donor/intermediate 层。
