#!/usr/bin/env python3
"""
清洗并转换黄酮结构多样性专用模板为 aizynthfinder 格式
（复用现有 convert_templates_final.py 逻辑，但处理新的 structural-diversity JSON 文件）
"""
import json
import re
import hashlib
...
TEMPLATE_FILES = [
    "templates_structural_class.json",
    "templates_glycosides.json",
    "templates_substituents.json",
    "templates_prenyl_cyclizations.json",
    "templates_quinone_da.json",
]

OUTPUT_DIR = Path("/home/ljx/retro_synthesis/templates/flavonoid_structural_diversity")
...
def main():
    ...
    for t in all_templates:
        ...
        if ok:
            ...
            rows.append({
                ...
                'classification': t.get('category', 'flavonoid_structural'),
                'library_occurence': 1,
                'reaction_id': rid,
                'name': name,
    ...
    if rows:
        ...
        hdf5_path = OUTPUT_DIR / 'flavonoid_structural_templates.hdf5'
        ...
        csv_path = OUTPUT_DIR / 'flavonoid_structural_templates.csv.gz'
        df.to_csv(csv_path, index=False, compression='gzip')
        print(f"已保存CSV: {csv_path}")

        meta_path = OUTPUT_DIR / 'flavonoid_structural_templates_metadata.csv'
    ...
    if failed:
        ...
        failed_path = OUTPUT_DIR / 'flavonoid_structural_templates_failed.csv'
